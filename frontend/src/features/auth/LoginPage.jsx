...
export default function LoginPage() {
  ...
  const handleDemoLogin = async () => {
    ...
    setServerError('')
    try {
      const demoData = { email: 'operator@rov.local', password: 'Operator@123' }
      const res = await api.post('/auth/login', demoData)
      const { user, accessToken, refreshToken } = res.data
      setAuth(user, accessToken, refreshToken)
      navigate('/trips/6a546254198ac240dc0ed995?detect=true')
    } catch (err) {
      const firstError = err.errors?.[0]?.msg
      setServerError(firstError || err.message || 'Demo login failed. Please try again.')
    } finally {
      setLoading(false)
    }
  }
    ...
    <div className="min-h-screen bg-gray-900 flex items-center justify-center px-4">
      <div className="bg-card rounded-2xl shadow-xl p-8 w-full max-w-md border border-border">
        ...
        <MarineButton
          ...
          className="w-full mt-3 border-cyan-500/30 text-cyan-400 hover:bg-cyan-500/10 font-semibold"
        >
          ⚡ Quick Demo Login (Operator)
        </MarineButton>
