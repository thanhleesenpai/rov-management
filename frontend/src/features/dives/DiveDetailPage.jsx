...
export default function DiveDetailPage() {
  ...
  useEffect(() => {
    ...
    // Re-calculate when video metadata loads (actual dimensions become available)
    const v = videoRef.current
    if (v) v.addEventListener('loadedmetadata', recalc)

    recalc()
    return () => {
      observer.disconnect()
      if (v) v.removeEventListener('loadedmetadata', recalc)
    }
  }, [containerRef, videoRef, selIdx])
