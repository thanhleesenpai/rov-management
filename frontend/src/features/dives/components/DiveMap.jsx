...
// isAnimating: when true, map auto-follows the ROV position
...
export default function DiveMap({
  ...
  const inst          = useRef(null)   // L.Map instance
  const polylineRef   = useRef(null)
  const rovMarkerRef  = useRef(null)
  const isMovingRef   = useRef(false)  // true while we programmatically pan (ignore dragstart)
  const userPannedRef = useRef(false)  // user manually dragged → disable auto-follow
  ...
  useEffect(() => {
    ...
    import('leaflet').then(({ default: L }) => {
      ...
      // Detect user drag — disable auto-follow until recenter
      map.on('dragstart', () => {
  ...
  // ── Reset user-pan state when animation stops ─────────────────────────────────
  useEffect(() => {
    if (!isAnimating) {
  ...
  // ── Live update — position, arrow rotation, auto-follow ───────────────────────
  useEffect(() => {
    ...
    if (dvlPath && dvlPath.length > 0) {
      ...
      // Auto-follow: pan to keep ROV centred while animating (unless user has dragged)
      if (isAnimating && !userPannedRef.current && inst.current) {
        isMovingRef.current = true
        inst.current.panTo(lastPos, { animate: true, duration: 0.3, easeLinearity: 1 })
        // Clear flag after pan completes (slightly longer than duration)
  ...
  }, [dvlPath, dvlYaw, isAnimating]) // eslint-disable-line react-hooks/exhaustive-deps

  // ── Recenter: re-enable auto-follow + pan immediately ────────────────────────
  function handleRecenter() {
    userPannedRef.current = false
    setShowRecenter(false)
    if (rovMarkerRef.current && inst.current) {
      isMovingRef.current = true
      inst.current.panTo(rovMarkerRef.current.getLatLng(), { animate: true, duration: 0.5 })
      setTimeout(() => { isMovingRef.current = false }, 600)
    }
  }
  ...
  return (
    <div className="w-full h-full relative">
      ...
      {/* Recenter button — appears when user has dragged away from ROV during animation */}
      {showRecenter && (
        <button
