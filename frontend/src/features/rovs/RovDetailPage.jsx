...
function StatCard({ icon: Icon, label, value, color }) {
  const colors = {
    ...
    orange: { bg: 'bg-orange-50 dark:bg-orange-900/20', icon: 'text-orange-500' },
  }
  ...
  return (
    <div className="max-w-3xl">
      ...
      <div className="bg-card rounded-xl shadow border border-border p-6 mb-4">
        ...
        {rov.notes && (
          <div className="mt-4 pt-4 border-t border-border text-sm text-muted-foreground whitespace-pre-wrap">{rov.notes}</div>
        )}
      </div>
      ...
      <div className="bg-card rounded-xl shadow border border-border p-5">
        ...
        {tripsLoading ? (
          ...
          <div className="text-center py-10 text-muted-foreground text-sm">No trips recorded for this ROV.</div>
        ) : (
              ...
              return (
                <div key={trip._id} className="flex items-center justify-between py-3 gap-3">
                  ...
                  <Link to={`/trips/${trip._id}`}
                    className="p-1.5 text-muted-foreground hover:text-primary rounded shrink-0 transition-colors" title="View trip">
                    <Eye size={15} />
                  </Link>
                </div>
              )
