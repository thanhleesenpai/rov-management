...
export default function SensorChart({ trip }) {
  const tripId = trip._id
  const [hidden, setHidden] = useState({})

  const { data, isLoading, isError } = useQuery({
    queryKey: ['sensor', tripId],
    queryFn: () => api.get(`/trips/${tripId}/sensor-data`).then(r => r.data),
    enabled: (trip.sensorCount || 0) > 0,
    staleTime: 5 * 60 * 1000,
  })
  ...
  return (
    <div className="space-y-4">
      ...
      {/* GPS badge — map will be in TripDetailPage (Task 6) */}
      {hasGps && (
        <div className="flex items-center gap-2 bg-muted rounded-lg px-3 py-2">
          <MapPin size={12} className="text-primary shrink-0" />
          <div className="min-w-0">
            {trip.locationName && (
              <p className="text-xs font-medium text-foreground truncate" title={trip.locationName}>
                {trip.locationName}
              </p>
            )}
