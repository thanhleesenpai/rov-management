...
export function RecordedAtEditor({ media, tripId, onCancel, onDone }) {
...
export function RetryAnalysisButton({ mediaId, tripId, queryClient }) {
  const [loading, setLoading] = useState(false)
  const handleRetry = async () => {
    setLoading(true)
    try {
      await api.post(`/media/${mediaId}/analyze`, { model: 'yolov8n', confidence: 0.3 })
      await queryClient.invalidateQueries({ queryKey: ['media', tripId] })
    } catch {
      setLoading(false)
    }
  }
...
export function AIAnalyzePopover({ media, tripId, canUse, portalTarget, onWillOpen, closeSignal, onOpenChange }) {
  const queryClient  = useQueryClient()
