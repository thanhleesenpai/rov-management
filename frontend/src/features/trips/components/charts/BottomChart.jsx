...
export function BottomChart({
  ...
  brushRange,        // { startIndex, endIndex } | null — controlled from parent
  onBrushChange,     // (range) => void — called when user drags brush
}) {
  const brushProps = {
    dataKey: 'timestamp',
    height: 14,
    ...
    // Controlled brush range — undefined means full range (Recharts default)
          ...
          <div className="relative w-full h-full">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart
                ...
                {!hidden.yaw && (
                  <Line type="monotone" dataKey="yaw"   name="Yaw"   stroke="#8b5cf6" strokeWidth={1.5} dot={false} isAnimationActive={false} />
                )}
                {!hidden.pitch && (
                  <Line type="monotone" dataKey="pitch" name="Pitch" stroke="#06b6d4" strokeWidth={1.5} dot={false} isAnimationActive={false} />
                )}
                {!hidden.roll && (
                  <Line type="monotone" dataKey="roll"  name="Roll"  stroke="#f97316" strokeWidth={1.5} dot={false} isAnimationActive={false} />
                )}
