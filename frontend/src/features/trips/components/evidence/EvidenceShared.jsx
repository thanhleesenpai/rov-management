...
export function EvidenceViewer({ evidence, media, tripId, onClose, queryClient, evidenceShowCenterPauseBtn, showEvidenceCenterPauseBriefly }) {
  ...
  const handleEvidenceAnalyze = useCallback(async () => {
    ...
    try {
      if (evidence.analysisStatus === 'pending') {
        ...
        queryClient.invalidateQueries({ queryKey: ['snapshots', tripId] })
      } else {
        // Start analysis
  ...
  }, [evidence, tripId, evidenceAnalyzeModel, evidenceAnalyzeConf, queryClient])
  ...
  // Shared helper: draw bbox labels onto an already-loaded Image and download as PNG
  const downloadCanvasWithBbox = useCallback((img, labels, suffix = 'detected') => {
    ...
    labels.forEach(l => {
      ...
      const pw = (x2 - x1) * cw, ph = (y2 - y1) * ch
      const color = l.confidence > 0.8 ? '#60a5fa' : '#fbbf24'
      ctx.strokeStyle = color
      ctx.lineWidth = Math.max(cw * 0.003, 2)
      ctx.strokeRect(px1, py1, pw, ph)
      const fs = Math.max(ch * 0.028, 12)
      const lh = fs * 1.6
      const text = `${l.name} ${Math.round(l.confidence * 100)}%`
      ctx.font = `700 ${fs}px system-ui, sans-serif`
      const labelW = ctx.measureText(text).width + fs * 1.2
    ...
    Object.assign(document.createElement('a'), {
      ...
      download: `evidence-${String(evidence._id).slice(-6)}-${suffix}.png`,
    }).click()
  }, [evidence])
  ...
  const handleEvidenceDownload = useCallback(async (format) => {
    ...
    try {
      if (format === 'png') {
        const labsWithBbox = showEvidenceDetect ? visibleEvidenceLabs.filter(l => l.bbox) : []

        if (isClip) {
          // For clips: extract current frame from parent video via backend (ffmpeg).
          // evidenceVideoRef.currentTime is the absolute time in the original video.
          const absoluteTime = evidenceVideoRef.current?.currentTime ?? (evidence.startTime || 0)
          const suffix = String(evidence._id).slice(-6)
          // Interceptor returns response.data directly, so for responseType:'blob' the result IS the Blob
          const frameBlob = await api.get(
            `/snapshots/${evidence._id}/frame-at?time=${absoluteTime.toFixed(3)}`,
            { responseType: 'blob' },
          )
          ...
          if (labsWithBbox.length > 0) {
            downloadCanvasWithBbox(img, labsWithBbox, `frame-${suffix}`)
          } else {
            // blobUrl already revoked — draw to canvas for a clean data URL
            const c = document.createElement('canvas')
            c.width = img.naturalWidth; c.height = img.naturalHeight
            c.getContext('2d').drawImage(img, 0, 0)
            Object.assign(document.createElement('a'), {
              href: c.toDataURL('image/png'),
              download: `clip-frame-${suffix}.png`,
            }).click()
          }
          return
        }

        // Photo: fetch stored image as blob (avoids CORS tainted-canvas) then burn bbox
        ...
        if (labsWithBbox.length > 0) {
          try {
            ...
            await new Promise((resolve, reject) => { img.onload = resolve; img.onerror = reject; img.src = blobUrl })
            URL.revokeObjectURL(blobUrl)
            downloadCanvasWithBbox(img, labsWithBbox)
            return
          } catch (canvasErr) {
            console.error('Canvas bbox export failed, falling back to plain download:', canvasErr)
          }
        }

        // Plain download (no bbox or canvas failed)
        const res = await fetch(url)
        const blob = await res.blob()
        const blobUrl = URL.createObjectURL(blob)
...
function EvidenceCard({ snap, canEdit, tripId, queryClient, onSeek, onClick, onDelete, selectMode, selected, onSelect }) {
...
export function EvidencePanel({ snapshots, isOpen, tripId, canEdit, videoRef, queryClient, currentMediaId, onSelectEvidence, confirmDelete, setConfirmDelete, isHorizontal = false }) {
