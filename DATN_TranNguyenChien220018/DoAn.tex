...
\hyphenation{Compose MongoDB YOLOv8 YOLOv8n FastAPI microservice frontend backend QGroundControl BlueOS ByteTrack Recharts Leaflet Zustand Passport Nominatim OpenStreetMap presigned middleware MJPEG thruster OpenCV PID}
...
\newpage
%\pagestyle{fancy} % Áp dụng header và footer
\chapter{ĐIỀU KHIỂN TỰ ĐỘNG} %Kết luận và hướng phát triển}
\label{chapter:conclusion}
\subfile{Chuong/6_Dieu_khien_tu_dong}

\newpage
%\pagestyle{fancy} % Áp dụng header và footer
\chapter{QUẢN LÝ THÔNG TIN VÀ PHÂN TÍCH DỮ LIỆU} %Kết luận và hướng phát triển}
\label{chapter:conclusion}
\subfile{Chuong/7_Quan_ly_ttin}

\newpage
%\pagestyle{fancy} % Áp dụng header và footer
\chapter{KẾT QUẢ THỰC NGHIỆM} %Kết luận và hướng phát triển}
\label{chapter:conclusion}
...
\subfile{Chuong/10_Luu_y_tai_lieu_tham_khao}
