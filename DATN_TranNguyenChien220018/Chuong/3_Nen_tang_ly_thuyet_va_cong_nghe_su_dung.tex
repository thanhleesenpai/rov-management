...
\begin{figure}[H]
    \centering
    \includegraphics[width =1.1\textwidth]{Hinhve/system_structure.png}
    \caption{Sơ đồ tổng quan hệ thống}
    \label{fig:usecase}
\end{figure}

Hệ thống tàu ngầm điều khiển từ xa (ROV -- Remotely Operated Vehicle) được thiết kế theo mô hình phân tán, bao gồm hai thành phần phần mềm chính: firmware nhúng chạy trực tiếp trên ROV và phần mềm điều khiển mặt đất Ground Control Station (GCS).
Firmware nhúng được triển khai trên nền tảng tính toán nhúng \texttt{QCS8550}, chạy hệ điều hành Linux và được phát triển chủ yếu bằng ngôn ngữ Python. Thành phần này chịu trách nhiệm giao tiếp với các cảm biến và cơ cấu chấp hành gắn trực tiếp trên ROV, thực hiện các thuật toán điều khiển thời gian thực, duy trì trạng thái ổn định của phương tiện dưới nước và truyền dữ liệu telemetry về trạm điều khiển.

Phần mềm Ground Control Station (GCS) được phát triển trên nền tảng desktop sử dụng ngôn ngữ \texttt{C\#} và framework WPF. GCS đóng vai trò trung tâm điều khiển, cho phép người vận hành giám sát trạng thái ROV theo thời gian thực, điều khiển bằng thiết bị Joystick, tiếp nhận và hiển thị dữ liệu video, lưu trữ dữ liệu nhiệm vụ, đồng thời hỗ trợ phân tích dữ liệu sau quá trình vận hành.

Kiến trúc hệ thống sử dụng kết nối Ethernet thông qua dây tether nối giữa ROV và trạm điều khiển. Trong hệ thống tồn tại ba luồng dữ liệu độc lập hoạt động song song.

Luồng thứ nhất là kênh điều khiển và telemetry giữa firmware trên ROV và GCS sử dụng giao thức MAVLink truyền qua UDP. Kênh này đảm nhiệm việc truyền lệnh điều khiển từ GCS đến ROV và truyền trạng thái cảm biến từ ROV ngược về GCS.

Luồng thứ hai là dữ liệu video từ camera gắn trên ROV. Camera kết nối trực tiếp với board QCS8550, hình ảnh được mã hóa JPEG và truyền liên tục về GCS dưới dạng luồng MJPEG thông qua giao thức TCP nhằm đảm bảo độ trễ thấp trong quá trình quan sát.

Luồng thứ ba bao gồm các thiết bị cảm biến độc lập như sonar quét hình ảnh và cảm biến định vị vận tốc DVL. Các thiết bị này giao tiếp mạng độc lập và truyền dữ liệu trực tiếp đến GCS mà không đi qua firmware điều khiển chính trên board QCS8550.

Kiến trúc phân tách này giúp giảm tải cho bộ xử lý nhúng trung tâm, đồng thời cho phép các thiết bị chuyên dụng như sonar và DVL hoạt động song song với hệ thống điều khiển mà không ảnh hưởng đến vòng lặp điều khiển thời gian thực của ROV.
...
\subsubsection{Bám giữ mục tiêu dựa trên Vision}
...
\hline
\textbf{Mô tả} & 
Theo dõi vùng mục tiêu do người vận hành lựa chọn và duy trì vị trí cùng kích thước biểu kiến của mục tiêu gần trạng thái tham chiếu ban đầu. \\
\hline
\textbf{Đầu vào} & 
Lệnh kích hoạt Hold Vision; vị trí hoặc vùng chọn mục tiêu từ giao diện; luồng ảnh MJPEG từ camera; dữ liệu heading và độ sâu từ các cảm biến trên ROV. \\
\hline
\textbf{Xử lý} & 
Thuật toán Optical Flow ước lượng dịch chuyển và thay đổi tỷ lệ của mục tiêu giữa các khung hình. Sai lệch ngang được chuẩn hóa để điều khiển trục sway; sai lệch tỷ lệ được dùng để điều khiển trục surge. Heading và độ sâu được duy trì bằng các vòng điều khiển cảm biến độc lập. Lệnh thị giác chỉ được sử dụng khi độ tin cậy đạt ngưỡng, dữ liệu chưa vượt timeout và người vận hành không tác động joystick. \\
\hline
\textbf{Kết quả} & 
ROV tự động bù chuyển động ngang và tiến--lùi để đưa mục tiêu trở lại gần trạng thái tương đối tại thời điểm lựa chọn, đồng thời duy trì hướng và độ sâu theo các chế độ được kích hoạt. Người vận hành luôn có quyền ưu tiên điều khiển thủ công. \\
\hline
\textbf{Trạng thái} & 
Đã triển khai ở mức nguyên mẫu; kết quả đánh giá thực nghiệm được bổ sung tại Chương 8. \\
\hline
\end{tabular}
\caption{Đặc tả chức năng bám giữ mục tiêu dựa trên Vision}
...
\subsubsection{Kiến trúc tổng thể phần mềm GCS}
...
\subsubsubsection\textbf{MavlinkService}
...
\subsubsubsection\textbf{CameraService}
...
\subsubsubsection\textbf{SonarService}
...
\subsubsubsection\textbf{DVLService}
...
\subsubsubsection\textbf{JoystickService}
...
\subsubsection{Nhóm module xử lý dữ liệu thời gian thực}

\subsubsubsection\textbf{TelemetryService}
...
\subsubsection{Nhóm module ghi dữ liệu nhiệm vụ}
...
\subsubsubsection\textbf{CameraRecorderService}
...
\subsubsubsection\textbf{SonarRecorder}
...
\subsubsubsection\textbf{MasterRecordingService}
...
\subsubsection{Nhóm module giao diện người dùng}
...
\subsubsubsection\textbf{Nhóm module Backend API}
...
\subsubsubsection\textbf{Nhóm module Frontend}
...
\subsubsubsection\textbf{Nhóm module xử lý bất đồng bộ}
...
\subsection{Quan hệ giữa các module}
